...
- `ConversationAgent` (`src/conversation/agent.ts`) — the central state machine. Owns inbound dedupe, group routing, buffering, the chat call, ordered outbound delivery, typing refreshes, status-driven queue advancement, and the optional read-receipt API call.
...
- `SendblueClient` (`src/sendblue/client.ts`) — outbound `send-message`, group send, reactions, `mark-read`, typing indicator. Each `send-message` includes its own `status_callback`; there is no global default. Non-2xx responses throw `SendblueApiError` exposing `operation`, `httpStatus`, `errorCode`, `serverMessage`, `responseBody` so callers can branch on documented codes (`5509` rate limit, `SMS_LIMIT_REACHED`, etc.) without regexing message strings.
- Status flow: `applyStatusUpdate` (`src/status/tracker.ts`) accumulates history; `ConversationAgent.handleStatus` advances the per-conversation outbound queue.
- `IdentityResolver` (`src/identity/resolver.ts`) is optional enrichment over `USER_LOOKUP_URL`, fail-open.
...
With `REDIS_URL`: conversation state, dedupe (`SET NX` with `DEDUPE_TTL_SECONDS`), outbound-handle → conversation-key map, and BullMQ-delayed buffer processing. **This is the production path.** Without it, in-memory maps and timers are used for tests and local smoke runs only — state is per-process and disappears on restart.

...
- `v0.3`: Rich messaging. Implemented with hosted media sends, iMessage send effects, Tapback reactions, contextual reply intent, direct read receipts, typing refreshes, XML tag compatibility, channel-aware suppression of iMessage-only features on SMS/downgraded conversations, and addressed inbound group routing. Note: Sendblue's send API has no documented routing-override parameter — a recipient whose number is iMessage-capable always receives iMessage, even if temporarily offline (Apple's network queues until reachable). The agent tracks `was_downgraded` for feature suppression only, not for outbound routing.
- `v0.4`: Groups and operations. Implemented partially with Sendblue contact upsert on inbound (`POST /api/v2/contacts`, gated by `SENDBLUE_CONTACTS_ENABLED`), Agent-plan limit tracking (per-line outbound rate, distinct-inbound, 24h reply window), bounded transient retry on rate-limit / server errors, per-line `SMS_LIMIT_REACHED` stall + scheduled retry, read-only `GET /admin/limits` telemetry, an operational visibility layer (Prometheus `/metrics`, request-scoped `traceId` propagation, extended `/health` + `/ready`, redacted-by-default `/admin/conversations`, `/admin/status`, `/admin/queue`, `/admin/dedupe` introspection routes — all token-gated under the existing `ADMIN_API_TOKEN`). Remaining: richer group operations / participant management, webhook registration CRUD. See `docs/features/contact-upsert.md`, `docs/features/plan-limits.md`, and `docs/features/operational-visibility.md`.
...
- Per-feature channel support matrix (verified against Sendblue docs on 2026-05-07):
  - Tapback reactions — **iMessage only.** "Reactions (tapbacks) are only supported for iMessage conversations. They are not supported for SMS or RCS." (https://docs.sendblue.com/api-v2/reactions/). Suppress on RCS, SMS, downgraded.
  - Outbound typing indicators / refreshes — **iMessage only.** "Typing indicators are only supported for iMessage conversations, not SMS or RCS." (https://docs.sendblue.com/api-v2/typing-indicators/). Suppress on RCS, SMS, downgraded.
  - Send effects (`send_style`) — **iMessage only.** Documented as "the iMessage expressive message style" with no other-channel coverage on https://docs.sendblue.com/api/resources/messages/methods/send/. Degrade inline (text still sends, effect stripped) on RCS, SMS, downgraded.
  - Read receipts (`POST /api/mark-read`) — **iMessage and RCS.** "Read receipts work for iMessage and RCS conversations, not SMS" (https://docs.sendblue.com/api-v2/read-receipts/). Suppress on SMS and downgraded.
  - Reply intent (contextual reply target) — **not exposed by Sendblue on any channel.** No `reply_to_message_handle`-style parameter is documented on send-message. The agent preserves the intent in chat actions and logs the resolved target, then delivers as a plain message.
  - Hosted media (`media_url`) — **all channels.** iMessage up to 100 MB; RCS images/video/audio; SMS as MMS (auto-compressed to ≤5 MB on downgrade per https://docs.sendblue.com/getting-started/sending-messages).
...
- Sendblue's send-message API has **no documented routing-override parameter** (no `force_sms`, `protocol`, `channel`, etc.). Sendblue routes by recipient capability detection: iMessage-capable numbers receive iMessage even when the device is temporarily offline (Apple's network queues until reachable). The `was_downgraded` field is response/webhook-only — sending it on `/api/send-message` is silently ignored. Do not pretend otherwise; if a deployment requires guaranteed SMS delivery, contact `support@sendblue.com`.
- Sendblue's per-message `status_callback` URLs arrive **without** the `sb-signing-secret` header. Validate `/webhook/status` leniently (`validateStatusCallbackSecret`); receive/typing/operational webhooks remain strictly validated.
- RCS feature gaps worth knowing about (Sendblue does not yet expose, even though Google's RBM protocol supports them): RCS Tapback reactions, RCS typing indicators, RCS suggested replies / quick-action buttons, RCS rich cards, threaded reply target. These are roadmap-asks for `support@sendblue.com`, not bugs in the agent.
...
- Sendblue's Contacts v2 API (`GET /api/v2/contacts/:phone_number`) is operator-managed contact metadata, not application user identity. Do not use it as a fallback for the identity resolver — application user identity remains a developer concern via `USER_LOOKUP_URL` or an injected `identityResolver`. The package does call `POST /api/v2/contacts` (upsert) when `SENDBLUE_CONTACTS_ENABLED=true` to push the resolver-supplied name into Sendblue's dashboard, but that is push-only and never read back as identity.
- Sendblue Agent-plan documented limits (verified 2026-05-08): per-line `1` msg/sec, `2,000`/hr, `4,000`/day; `1,000` distinct inbound contacts/day; `200` out-of-window follow-ups/day (the 24h reply window after each inbound is unlimited); `1,500`-message queue → 429. Sendblue does not document the SMS-limit reset boundary; the agent stalls the per-line queue and retries every `SMS_LIMIT_RETRY_INTERVAL_MS` (default 1h) up to `SMS_LIMIT_MAX_ATTEMPTS` rather than relying on a known boundary.
- Transient Sendblue error codes (`5509`, `5003`, `4001`, `5000`, `10002`, plus bare 429 / 5xx and `httpStatus: 0` network failures) are retried by the agent with bounded exponential backoff (default 3 attempts, 1s/4s/16s with jitter). The previous behavior — abort the queue on any ERROR — is now reserved for `validation` / `blacklist` / `unknown` categories or after retry exhaustion. Synchronous `SendblueApiError` from `sendMessage` and async ERROR status callbacks are both handled. Each scheduled retry captures `{ itemId, retryCount }` at scheduling time and `runRetry` drops the attempt at debug if the queued item changed (e.g. delivery-timeout-then-retry race or interrupt-then-retry race).
- `SMS_LIMIT_REACHED` stalls write through to `LimitCounterStore.setSmsLimitStall` with `{ attempts, nextRetryAt, conversationKey }`. `/admin/limits.smsLimitStallAttempts` is live, the cap honors `SMS_LIMIT_MAX_ATTEMPTS` (default 24), and the persisted state lets a fresh process resume scheduled retries on boot via `ConversationAgent.recoverPendingRetries()` (called non-blocking from `createApp`).
- `interruptSending` and `transitionToIdle` cancel any pending transient-retry timer and SMS-limit stall (both in-process scheduler + persisted store) eagerly. `advanceQueue` clears the SMS-limit stall on the next successful `DELIVERED`/`SENT` so the next 5509 starts a fresh attempt counter.
- `loadConfig` rejects misconfigurations at boot: `*_WARN_THRESHOLD > *_LIMIT` (inbound contacts, follow-ups), `OUTBOUND_RATE_LIMIT_PER_HOUR > OUTBOUND_RATE_LIMIT_PER_DAY`, `TRANSIENT_RETRY_BASE_MS > TRANSIENT_RETRY_MAX_MS`, and zero-valued `SENDBLUE_CONTACTS_DEDUPE_TTL_SECONDS` / `SMS_LIMIT_RETRY_INTERVAL_MS` / `SMS_LIMIT_MAX_ATTEMPTS`. Misconfig fails fast instead of silently swallowing warn-log emissions.
- Sendblue contact upsert fires only on **invoked & authorized** group inbound. Non-invoked groups and unauthorized invokers (when `VALID_USER_REQUIRED=true`) return early before the upsert hook, matching the broader "groups stay silent unless addressed" rule.
...
- Optional identity resolution is enrichment, not admission control. The agent catches any error from `resolveByPhone`, logs at warn level, and proceeds with `identity: null`. Resolvers should throw on transport, timeout, non-2xx, and parse errors so the agent log captures them. The resolver response shape now includes optional `firstName`, `lastName`, `tags`, and `customVariables` fields used by the contact-upsert hook; existing `{ userId, data, authorized }` resolvers remain backward compatible.
...
- Operational visibility is in-tree, dependency-free, and provider-neutral. Metrics flow through `MetricsCollector` (`src/metrics/`) with hand-rolled Prometheus text export; correlation is a request-scoped `traceId` (`src/http/trace.ts`) persisted on the conversation record and outbound handle mapping. Do not pull in external metrics/tracing SDKs — keep adapters at the call-site boundary so an OpenTelemetry/Sentry plug-in is a wrapper, not a coupling. Label cardinality is bounded by design: error-code labels normalize through `normalizeErrorCodeLabel`; conversation/message/phone identifiers belong in logs (with traceId), never in metric labels.
- Token gating reuses `ADMIN_API_TOKEN` and the shared `validateAdminToken` (`src/http/auth.ts`). New operator-only routes mount **only** when the token is set, so a misconfigured deploy cannot accidentally expose live state. PII-bearing introspection (`/admin/conversations`, `/admin/status`) redacts content and phone numbers by default; `?reveal=true` opts back into the raw record.
